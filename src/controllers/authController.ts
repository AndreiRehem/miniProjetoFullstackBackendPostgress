...
import { AuthRequest } from "../middlewares/authMiddleware"; // interface estendida
...
export const register = async (req: Request, res: Response) => {
  ...
  if (!name || !email || !password) {
    return res
      .status(400)
      .json({ message: "Todos os campos (name, email, password) são obrigatórios." });
  }

  try {
    const user = await authService.registerUser(name, email, password);

    // Sequelize usa 'id', não '_id'
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erro ao tentar fazer login.", error: errorMessage });
  }
};
